    ...
    function dateSelect($compile) {
        ...
        var monthYearTemplate = '<div ng-form="dateCursorForm" class="date-select">' +
                    '<div class="input-group">' +
                        ...
                        '</span>' +
                        '<strong>' +
                            '<input name="dateCursor" type="text" class="form-control text-center strong" active-save="updateDateCursor" active-save-mode="instant" date-format format="month" required ng-model="model" />' +
                        '</strong>' +
                        '<span class="input-group-btn">' +
                            '<button class="btn btn-invisible glyphicon glyphicon-record" type="button" translate-once-title="GENERAL.DATE_SELECT.TODAY" ng-click="today()" />' +
                            '<button class="btn btn-invisible glyphicon glyphicon-menu-right" type="button" translate-once-title="GENERAL.DATE_SELECT.FORWARD" ng-click="forward()" />' +
            ...
            link: function (scope, element, attrs) {
                ...
                scope.back = function () {
                    if (scope.model === null || scope.model === undefined) {
                        ...
                        if (scope.datepickerMode == 'month') {
                            setDate(new Date(scope.model).addMonths(-1));
                        }
                        else {
                ...
                scope.today = function () {
                    if (scope.datepickerMode == 'month') {
                        setDate(new Date().toDateOnlyFirstDayInMonth());
                    }
                    else {
