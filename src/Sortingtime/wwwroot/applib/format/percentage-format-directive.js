(function () {
    ...
    angular
        .module('app')
        .directive("percentageFormat", percentageFormat)

    percentageFormat.$inject = ['$locale', '$filter'];

    function percentageFormat($locale, $filter) {
        return {
            restrict: 'A',
            require: ['ngModel'], 
            compile: function (tElement, tAttrs) {
                if (tElement[0].nodeName !== 'INPUT') {
                    throw ('Error. percentageFormat directive must be used inside an <input> element.');
                }            
                ...
                return function (scope, element, attrs, ctrls) {
                    ...
                    // VALIDATORS
                    ngModel.$validators.percentage = function (modelValue, viewValue) {
                        if (debugLog) console.log("validators.percentage modelValue: " + modelValue + ", viewValue: " + viewValue);

                        if (viewValue === undefined || viewValue === null || viewValue === '') {
                            return true;
                        }
                        if (isValidPercentage(viewValue)) {
                            return true;
                        }
                        return false;
