...
namespace Sortingtime.Infrastructure.Translation
{
    public class LocaleDa : Ilocale
    {
        ...
        {
            using (var reader = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("Sortingtime.Shared.Infrastructure.Translation.Da.json")))
            {
        ...
        public string Translate(string key1, string key2)
        {
            ...
            {
                throw new Exception($"Da Translate missing [key1:'{key1}', key2:'{key2}']", exc);
            }
        }
    }
}
